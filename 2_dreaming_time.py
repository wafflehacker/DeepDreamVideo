...
#Load DNN
model_path = '/usr/local/caffe/models/bvlc_googlenet/' # substitute your path here
...
def handler(frames):
	...
	make_sure_path_exists(sys.argv[4])
	print "Opening and converting "+sys.argv[2]+"/"+frames
	nameframe = sys.argv[2]+"/"+frames
	frame = np.float32(PIL.Image.open(nameframe))
	frame = deepdream(net, frame, iter_n = 5)
	print "Saving to "+sys.argv[4]+"/"+frames
	saveframe=sys.argv[4]+"/"+frames
	PIL.Image.fromarray(np.uint8(frame)).save(saveframe)
	newframe= sys.argv[2]+"/"+frames
...
if __name__ == "__main__":
    ...
    pool.map(handler, os.listdir(args.input))
